...
class GroupController extends Controller {
    ...
    async create() {
        ...
        if (!friends.length) {
            return this.error(400, '请选择需要加入群聊的好友');
        }
        // 创建群聊
    ...
    async info() {
        ...
        if (index === -1) {
            return this.error(400, '你不是该群成员，没有权限');
        }
    ...
    async rename() {
        ...
        let index = group.group_users.findIndex(item => item.user_id === current_user_id);
        if (index === -1) {
            return this.error(400, '你不是该群成员');
        }
        // 验证是否是群主
        if (group.user_id !== current_user_id) {
            return this.error(400, '你不是管理员，没有权限');
        }
        // 修改群名称
        group.name = name;
        await group.save();
    ...
    async invite() {
        ...
        let index2 = group.group_users.findIndex(item => item.user_id === user_id);
        if (index2 !== -1) {
            return this.error(400, '对方已经是该群成员');
        }
        // 对方是否存在
        ...
        if (!user) {
            return this.error(400, '对方不存在或者已被封禁');
        }
    ...
    async join() {
        ...
        let index = group.group_users.findIndex(item => item.user_id === current_user_id);
        if (index !== -1) {
            return this.error(400, '你已经是该群成员');
        }
        // 加入该群
        await app.model.GroupUser.create({
            user_id: current_user_id,
