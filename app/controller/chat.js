...
class ChatController extends Controller {
    ...
    async detail() {
        ...
        switch (type) {
            case 'user':
                ret = await service.user.friend(user_id, id)
                if (ret.suc === false) {
                    return this.error(400, ret.msg)
                }
                return this.success({
                    id,
                    name:ret.data.userInfo.nickname ? ret.data.userInfo.nickname : ret.data.userInfo.username,
                    avatar:ret.data.userInfo.avatar
                });
            case 'group':
                ret = await service.group.info(user_id, id);
                if (ret.suc === false) {
                    return this.error(400, ret.msg)
                }
                return this.success({
    ...
    async send() {
        ...
        ctx.validate({
            ...
            chat_type: { type: 'enum', values: ['user', 'group'] },
        ...
        if (chat_type === 'user') {
            ...
            if (!Friend) {
                return this.error(400, '对方不存在或者已经把你拉黑');
            }
            // 验证好友是否被禁用
            if (!Friend.userInfo.status) {
                return this.error(400, '对方已被禁用');
            }
            // 构建消息格式
