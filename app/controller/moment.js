...
class MomentController extends Controller {
    ...
    async like() {
        ...
        if (!MomentTimeline) {
            return this.error(400, '朋友圈消息不存在');
        }
        ...
        if (like) {
            await like.destroy();
            this.success(MomentTimeline.moment.moment_likes);
        } else {
    ...
    async comment() {
        ...
        ctx.validate({
            ...
            reply_id: {type: "int",required: true}
        });
    ...
    async list() {
        ...
        if (!user_id) {
            ...
            // 用户是否存在
            if (!f) {
                return this.error(400, '用户不存在或已被禁用');
            }
            // 是否是好友关系
